document.getElementById('occupation').addEventListener('change', function() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      // Create an intersection observer to trigger the next batch after every 6th card has scrolled through
      const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            // If the 6th card is in view, load the next batch
            if (currentIndex < cardData.length) {
              loadCards(cardData, currentIndex, batchSize); // Load more cards
            }
        ...
        rootMargin: '0px 0px 100px 0px',  // Trigger when 6th card is about to come into view
        threshold: 0.1  // Trigger when 10% of the 6th card is in view
      });
    
      // Observe the 6th card (index 5)
      const observeCard = () => {
        const sixthCard = document.querySelectorAll('.properties-items')[5];
        if (sixthCard) {
          observer.observe(sixthCard); // Start observing the 6th card
        }
      };
    
      // Initialize by observing the 6th card
      observeCard();
    
      // Optional: Reassign observer when new cards are loaded
      window.addEventListener('scroll', function() {
        if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 100) {
          if (currentIndex < cardData.length) {
            loadCards(cardData, currentIndex, batchSize);  // Load more cards
            observeCard();  // Re-observe after loading more cards
          }
        }
      });
    });
