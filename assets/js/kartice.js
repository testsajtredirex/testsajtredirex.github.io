...
websiteLink: "" // No website link provided
  ...
  let cardData = [...originalCardData];

  // Funkcija za prikazivanje svih kartica
  function loadCards(cardData) {
    propertiesBox.innerHTML = ''; // Očisti prethodne kartice
  ...
  function createCard(cardInfo) {
    ...
    card.innerHTML = `
      <div class="item ${cardInfo.city.toLowerCase()} ${cardInfo.occupation.toLowerCase()}">
        ...
        ${cardInfo.personalPageLink ? `
          <div class="main-button-info" style="width: 90%; margin: 0 auto;">
            <a href="${cardInfo.personalPageLink}" style="display: block; text-align: center; color: white;">Detaljnije</a>
          </div>` : ''}
  ...
  // Učitaj sve kartice odmah
  loadCards(cardData);

  // Event listener za promenu filtra
  document.getElementById('occupation').addEventListener('change', function () {
    const selectedOccupation = this.value;

    if (selectedOccupation) {
      // Filtriraj kartice prema izboru
      cardData = originalCardData.filter(card => card.occupation.toLowerCase() === selectedOccupation.toLowerCase());
      loadCards(cardData);
    } else {
      // Ako nije izabran filter, učitaj sve kartice
